...
const SoilLimit: React.FC = () => {
  const [isPopupOpen, setIsPopupOpen] = useState<boolean>(false);
  const [minSoil, setMinSoil] = useState<number | null>(null);
  const [maxSoil, setMaxSoil] = useState<number | null>(null);
    ...
    <div className="p-4 bg-gradient-to-r from-green-100 to-green-250 rounded-lg dark:from-green-200 dark:to-green-400">
      ...
      <div className="flex items-center justify-between bg-white p-4 rounded-lg shadow-md">
        <div className="flex items-center">
          ...
          <div>
            <p className="font-bold text-lg">Đặt Ngưỡng Độ ẩm đất</p>
            <p className="text-sm text-gray-500">
              Thiết lập ngưỡng cho cảm biến Độ ẩm đất
            </p>
          </div>
        </div>
        <button
        ...
        <div className="fixed inset-0 bg-opacity-50 lg:ml-70 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-2xl shadow-lg w-[90%] max-w-md">
            ...
            <div className="mb-4">
              ...
              <input
                type="number"
                placeholder="Nhập Độ ẩm đất tối thiểu"
                value={minSoil ?? ""}
                onChange={(e) =>
                  setMinSoil(
            ...
            <div className="mb-4">
              ...
              <input
                type="number"
                placeholder="Nhập Độ ẩm đất tối đa"
                value={maxSoil ?? ""}
                onChange={(e) =>
                  setMaxSoil(
                    e.target.value ? Number(e.target.value) : null
